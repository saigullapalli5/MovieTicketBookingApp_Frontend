...
const SeatsPage = () => {
  ...
  const handleCheckout = async () => {
    try {
      ...
      // Calculate totals
      const lowerTotal = selectedSeats?.lower.length * theatreDetails?.lowerSeatPrice;
      const middleTotal = selectedSeats?.middle.length * theatreDetails?.middleSeatPrice;
      const balconyTotal = selectedSeats?.balcony.length * theatreDetails?.balconySeatPrice;
      
      // Set totals
      bookedSeatDetails.lower["total"] = lowerTotal;
      bookedSeatDetails.middle["total"] = middleTotal;
      bookedSeatDetails.balcony["total"] = balconyTotal;
      ...
      try {
        ...
        if (!bookingData?.status) {
          throw new Error(bookingData?.msg || 'Failed to create booking');
        }

        // Update show tickets
        const host = `${render}/api/shows/updateshowtickets/${showId}`;
        const seatsToUpdate = JSON.parse(JSON.stringify(bookedSeatDetails));
        delete seatsToUpdate.balcony.total;
        delete seatsToUpdate.middle.total;
        delete seatsToUpdate.lower.total;
        
        const res = await axios.put(host, seatsToUpdate);
        ...
        if (res?.data?.status) {
          ...
          throw new Error(res.data?.msg || 'Failed to update show tickets');
        }
      } catch (error) {
